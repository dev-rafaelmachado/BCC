...
char* mes_str[NUM_MESES] = { "janeiro", "fevereiro", "março", "abril", "maio", "junho", "julho", "agosto", "setembro", "outubro", "novembro", "dezembro"};
...
int main()
{
  ...
  int mes_selecionado, indexPintoresTrue = 0;
  ...
  for (int index_Meses =1; index_Meses <= 12; index_Meses++){
    printf("\t(%2d) %s\n", index_Meses, mes_str[index_Meses-1]);
  }
  ...
  while (mes_selecionado > 12 || mes_selecionado < 1){
    ...
    printf("Pintores nascidos em %s:\n",mes_str[mes_selecionado-1]);
  for(int i = 0; i < NUM_PINTORES; i++){
    if (pintor[i].mes == mes_selecionado-1){
      indexPintoresTrue++;
      printf("(%d) %s\n",indexPintoresTrue, pintor[i].nome);
    }
  }
  if(indexPintoresTrue == 0){
    puts("\n==> Nenhum Pintor foi encontrado");
  } else if (indexPintoresTrue == 1){
    puts("\n==> Apenas um pintor foi encontrado");
  } else {
    printf("\n==> Foram encontrados %d pintores", indexPintoresTrue);
  }
